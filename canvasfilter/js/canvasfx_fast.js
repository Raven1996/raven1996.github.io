...
function normalBlurEffect(radius, lggamma, mask, type, centerX, centerY){
	//这个radius当比例来用 0表示没有模糊。
	radius = Math.log(radius*10000+1)/Math.log(1000001);
	var imgData = ctxA.getImageData(0, 0, canvasA.width, canvasA.height);
	var pxData = imgData.data;
	...
	for(var i = 0; i < fullH; i++)
		for(var j = 0; j < fullW; j++){
			var p = i*fullW + j;
			p = p << 2;
			pxArr[p+0] = Math.exp(Math.log(pxData[p+0]/255) * power);
			pxArr[p+1] = Math.exp(Math.log(pxData[p+1]/255) * power);
			pxArr[p+2] = Math.exp(Math.log(pxData[p+2]/255) * power);
		}
	...
	for(var i = 0; i < fullH; i++){
		var p = (i * fullW) << 2;
		var r = pxArr[p+0], g = pxArr[p+1], b = pxArr[p+2];
		var rg = 0, gg = 0, bg = 0; //切向斜率
		tmpPxArr[p+0] = r;
		tmpPxArr[p+1] = g;
		tmpPxArr[p+2] = b;
		for(var j = 1; j < fullW; j++){
			var pp = (i*fullW + j) << 2;
			if ((pxArr[pp+0]-r)*rg > 0) {
				rg = rg * radius + (pxArr[pp+0]-r) * (1-radius)
				if (Math.abs(rg) > Math.abs(pxArr[pp+0]-r)) rg = pxArr[pp+0]-r;
			}else rg = (pxArr[pp+0]-r) * (1-radius);
			if ((pxArr[pp+1]-g)*gg > 0) {
				gg = gg * radius + (pxArr[pp+1]-g) * (1-radius)
				if (Math.abs(gg) > Math.abs(pxArr[pp+1]-g)) gg = pxArr[pp+1]-g;
			}else gg = (pxArr[pp+1]-g) * (1-radius);
			if ((pxArr[pp+2]-b)*bg > 0) {
				bg = bg * radius + (pxArr[pp+2]-b) * (1-radius)
				if (Math.abs(bg) > Math.abs(pxArr[pp+2]-b)) bg = pxArr[pp+2]-b;
			}else bg = (pxArr[pp+2]-b) * (1-radius);
			r += rg;
			g += gg;
			b += bg;
			tmpPxArr[pp+0] = r;
		...
		p = (i * fullW + fullW - 1) << 2;
		r = pxArr[p+0], g = pxArr[p+1], b = pxArr[p+2];
		rg = 0, gg = 0, bg = 0; //切向斜率
		tmpPxArr[p+0] = (tmpPxArr[p+0]+r) * 0.5;
		tmpPxArr[p+1] = (tmpPxArr[p+1]+g) * 0.5;
		tmpPxArr[p+2] = (tmpPxArr[p+2]+b) * 0.5;
		for(var j = fullW - 2; j >= 0; j--){
			var pp = (i*fullW + j) << 2;
			if ((pxArr[pp+0]-r)*rg > 0) {
				rg = rg * radius + (pxArr[pp+0]-r) * (1-radius)
				if (Math.abs(rg) > Math.abs(pxArr[pp+0]-r)) rg = pxArr[pp+0]-r;
			...
			g += gg;
			b += bg;
			r = r * radius + pxArr[pp+0] * (1 - radius);
			g = g * radius + pxArr[pp+1] * (1 - radius);
			b = b * radius + pxArr[pp+2] * (1 - radius);
			tmpPxArr[pp+0] = (tmpPxArr[pp+0]+r) * 0.5;
			tmpPxArr[pp+1] = (tmpPxArr[pp+1]+g) * 0.5;
			tmpPxArr[pp+2] = (tmpPxArr[pp+2]+b) * 0.5;
		}
	}
	...
	for(var j = 0; j < fullW; j++){
		var p = j << 2;
		var r = tmpPxArr[p+0], g = tmpPxArr[p+1], b = tmpPxArr[p+2]; 
		var rg = 0, gg = 0, bg = 0; //切向斜率
		pxArr[p+0] = r;
		pxArr[p+1] = g;
		pxArr[p+2] = b;
		for(var i = 1; i < fullH; i++){
			var pp = (i*fullW + j) << 2;
			if ((tmpPxArr[pp+0]-r)*rg > 0) {
				rg = rg * radius + (tmpPxArr[pp+0]-r) * (1-radius)
				if (Math.abs(rg) > Math.abs(tmpPxArr[pp+0]-r)) rg = tmpPxArr[pp+0]-r;
			}else rg = (tmpPxArr[pp+0]-r) * (1-radius);
			if ((tmpPxArr[pp+1]-g)*gg > 0) {
				gg = gg * radius + (tmpPxArr[pp+1]-g) * (1-radius)
				if (Math.abs(gg) > Math.abs(tmpPxArr[pp+1]-g)) gg = tmpPxArr[pp+1]-g;
			}else gg = (tmpPxArr[pp+1]-g) * (1-radius);
			if ((tmpPxArr[pp+2]-b)*bg > 0) {
				bg = bg * radius + (tmpPxArr[pp+2]-b) * (1-radius)
				if (Math.abs(bg) > Math.abs(tmpPxArr[pp+2]-b)) bg = tmpPxArr[pp+2]-b;
			}else bg = (tmpPxArr[pp+2]-b) * (1-radius);
			r += rg;
			g += gg;
			b += bg;
			pxArr[pp+0] = r;
		...
		r = pxArr[p+0], g = pxArr[p+1], b = pxArr[p+2];
		rg = 0, gg = 0, bg = 0; //切向斜率
		pxArr[p+0] = (pxArr[p+0]+r) * 0.5;
		pxArr[p+1] = (pxArr[p+1]+g) * 0.5;
		pxArr[p+2] = (pxArr[p+2]+b) * 0.5;
		for(var i = fullH- 2; i >= 0; i--){
			var pp = (i*fullW + j) << 2;
			if ((tmpPxArr[pp+0]-r)*rg > 0) {
				rg = rg * radius + (tmpPxArr[pp+0]-r) * (1-radius)
				if (Math.abs(rg) > Math.abs(tmpPxArr[pp+0]-r)) rg = tmpPxArr[pp+0]-r;
			...
			g += gg;
			b += bg;
			pxArr[pp+0] = (pxArr[pp+0]+r) * 0.5;
			pxArr[pp+1] = (pxArr[pp+1]+g) * 0.5;
			pxArr[pp+2] = (pxArr[pp+2]+b) * 0.5;
		}
	}
	...
	for(var i = 0; i < fullH; i++)
		for(var j = 0; j < fullW; j++){
			var p = i*fullW + j;
			p = p << 2;
			pxData[p+0] = Math.exp(Math.log(pxArr[p+0]) * power) * 255;
			pxData[p+1] = Math.exp(Math.log(pxArr[p+1]) * power) * 255;
			pxData[p+2] = Math.exp(Math.log(pxArr[p+2]) * power) * 255;
			pxData[p+3] = 255;
		}
